...
if snpmAnalysis.anova.type~="no ANOVA"
    ...
    if ischar(anova.effectNames)
        ...
        displayAnova(anova.Fcontinuum,anova.Fthreshold,anova.Fsignificant{1},Fs,xlab,ylab,ylimits,dimensions,nTicksX,nTicksY,xlimits,colorMap,imageSize,imageFontSize,equalAxis,deleteAxis,statLimit)
        ...
        for k=1:numel(anova.effectNames)
            ...
            displayAnova(anova.Fcontinuum{k},anova.Fthreshold{k},anova.Fsignificant{k},Fs,xlab,ylab,ylimits,dimensions,nTicksX,nTicksY,xlimits,colorMap,imageSize,imageFontSize,equalAxis,deleteAxis,statLimit)
...
if min(dimensions)==1
    ...
    for np=1:numel(posthoc)
        ...
        for comp=1:numel(posthoc{np}.differences.continuum)
            ...
            % differences
            plotmean({posthoc{np}.differences.continuum{comp}},CI,xlab,ylab,Fs,xlimits,nTicksX,[],[],imageFontSize,imageSize,transparancy1D,[],[],[],0,0)
            if size(posthoc{np}.differences.names,1)>1
                ...
                exportgraphics(gcf,fullfile(savedir2, 'DIFF', [verifSaveName([char(posthoc{np}.differences.names{1,comp}) ' - ' char(posthoc{np}.differences.names{2,comp})]) '.tif']),'Resolution',imageResolution)
                ...
                exportgraphics(gcf,fullfile(savedir2, 'DIFF', [verifSaveName(posthoc{np}.differences.names{comp}) '.tif']),'Resolution',imageResolution)
                savefig([savedir2 '/FIG/DIFF/' verifSaveName(posthoc{np}.differences.names{comp})])
            end
            close

            % relative differences
            plotmean({posthoc{np}.differences.continuumRelative{comp}},CI,xlab,'Differences (%)',Fs,xlimits,nTicksX,[],[],imageFontSize,imageSize,transparancy1D,[],[],[],0,0)
            if size(posthoc{np}.differences.names,1)>1
                legend([posthoc{np}.differences.names{1,comp} ' - ' posthoc{np}.differences.names{2,comp}],'Location','eastoutside','box','off')
                exportgraphics(gcf,fullfile(savedir2, 'DIFF', [verifSaveName([char(posthoc{np}.differences.names{1,comp}) ' - ' char(posthoc{np}.differences.names{2,comp})]) '%.tif']),'Resolution',imageResolution)
                savefig([savedir2 '/FIG/DIFF/' verifSaveName([char(posthoc{np}.differences.names{1,comp}) ' - ' char(posthoc{np}.differences.names{2,comp}) '%'])])
            ...
            plotTtest(posthoc{np}.tTests.Tcontinuum{1,comp},posthoc{np}.tTests.Tthreshold{comp},posthoc{np}.tTests.Tsignificant{1,comp},Fs,xlab,ylab,ylimits,dimensions,nTicksX,nTicksY,xlimits,imageFontSize,imageSize)
            if size(posthoc{np}.differences.names,1)>1
                legend([posthoc{np}.differences.names{1,comp} ' - ' posthoc{np}.differences.names{2,comp}],'Location','eastoutside','box','off')
                exportgraphics(gcf,fullfile(savedir2, 'SnPM', [verifSaveName([char(posthoc{np}.differences.names{1,comp}) ' - ' char(posthoc{np}.differences.names{2,comp})]) '.tif']),'Resolution',imageResolution)
                ...
                legend(posthoc{np}.differences.names{comp},'Location','eastoutside','box','off')
                exportgraphics(gcf,fullfile(savedir2, 'SnPM', [verifSaveName(posthoc{np}.differences.names{comp}) '.tif']),'Resolution',imageResolution)
            ...
            if size(posthoc{np}.differences.names,1)>1
                legend([posthoc{np}.differences.names{1,comp} ' - ' posthoc{np}.differences.names{2,comp}],'Location','eastoutside','box','off')
                exportgraphics(gcf,fullfile(savedir2, 'ES', [verifSaveName([char(posthoc{np}.differences.names{1,comp}) ' - ' char(posthoc{np}.differences.names{2,comp})]) '.tif']),'Resolution',imageResolution)
                savefig([savedir2 '/FIG/ES/' verifSaveName([char(posthoc{np}.differences.names{1,comp}) ' - ' char(posthoc{np}.differences.names{2,comp})])])
            else
                legend(posthoc{np}.differences.names{comp},'Location','eastoutside','box','off')
                exportgraphics(gcf,fullfile(savedir2, 'ES', [verifSaveName(posthoc{np}.differences.names{comp}) '.tif']),'Resolution',imageResolution)
    ...
    if numel(posthoc)==1
        ...
        plotmean(posthoc{np}.data.continuum,CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,xLine,yLine,xGrid,yGrid)
        legend(posthoc{np}.data.names,'Location','eastoutside','box','off')
        exportgraphics(gcf,fullfile(savedir2, [verifSaveName(posthoc{np}.name) '.tif']),'Resolution',imageResolution)
        savefig([savedir2 '/FIG/' verifSaveName(posthoc{np}.name)])
        close

        if snpmAnalysis.anova.type~="no ANOVA" % plot with aov
            plotmeanSnPM(posthoc{1}.data.continuum,posthoc{1}.tTests.Tcontinuum,posthoc{1}.tTests.Tsignificant,posthoc{1}.data.names,posthoc{1}.differences.names,CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,anova.Fsignificant,{posthoc{1}.name},ratioSnPM,SnPMPos,aovColor,xLine,yLine,xGrid,yGrid)
            exportgraphics(gcf,fullfile(savedir2, [verifSaveName(posthoc{1}.name) ' + SnPM.tif']),'Resolution',imageResolution)
            savefig([savedir2 '/FIG/' verifSaveName(posthoc{1}.name) ' + SnPM'])
            close
        end

        plotmeanSnPM(posthoc{1}.data.continuum,posthoc{1}.tTests.Tcontinuum,posthoc{1}.tTests.Tsignificant,posthoc{1}.data.names,posthoc{1}.differences.names,CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,[],[],ratioSnPM,SnPMPos,aovColor,xLine,yLine,xGrid,yGrid)
        exportgraphics(gcf,fullfile(savedir2, [verifSaveName(posthoc{1}.name) ' + SnPMnoAOV.tif']),'Resolution',imageResolution)
        savefig([savedir2 '/FIG/' verifSaveName(posthoc{1}.name) ' + SnPMnoAOV'])
        close

        if snpmAnalysis.anova.type~="no ANOVA" % plot with aov
            plotmeanSnPMsub(posthoc{1}.data.continuum,posthoc{1}.tTests.Tcontinuum,posthoc{1}.tTests.Tsignificant,posthoc{1}.data.names,posthoc{1}.differences.names,CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,anova.Fsignificant,{posthoc{1}.name},ratioSnPM,SnPMPos,aovColor,xLine,yLine,xGrid,yGrid)
            exportgraphics(gcf,fullfile(savedir2, [verifSaveName(posthoc{1}.name) ' + SnPMsub.tif']),'Resolution',imageResolution)
            savefig([savedir2 '/FIG/' verifSaveName(posthoc{1}.name) ' + SnPMsub'])
            close
        end

        plotmeanSnPMsub(posthoc{1}.data.continuum,posthoc{1}.tTests.Tcontinuum,posthoc{1}.tTests.Tsignificant,posthoc{1}.data.names,posthoc{1}.differences.names,CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,[],[],ratioSnPM,SnPMPos,aovColor,xLine,yLine,xGrid,yGrid)
        exportgraphics(gcf,fullfile(savedir2, [verifSaveName(posthoc{1}.name) ' + SnPMsubNoAOV.tif']),'Resolution',imageResolution)
        ...
        for np=1:pMax
            ...
            exportgraphics(gcf,fullfile(savedir2,  verifSaveName(anova.effectNames{np}), [verifSaveName(anova.effectNames{np}) ' + SnPM.tif']),'Resolution',imageResolution)
            savefig([savedir2 verifSaveName(anova.effectNames{np}) '/FIG/' verifSaveName(anova.effectNames{np}) ' + SnPM'])
            close

            plotmeanSnPM(posthoc{np}.data.continuum,posthoc{np}.tTests.Tcontinuum,posthoc{np}.tTests.Tsignificant,posthoc{np}.data.names,posthoc{np}.differences.names,CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,[],[],ratioSnPM,SnPMPos,aovColor,xLine,yLine,xGrid,yGrid)
            exportgraphics(gcf,fullfile(savedir2,  verifSaveName(anova.effectNames{np}), [verifSaveName(anova.effectNames{np}) ' + SnPMnoAOV.tif']),'Resolution',imageResolution)
            ...
            exportgraphics(gcf,fullfile(savedir2,  verifSaveName(anova.effectNames{np}), [verifSaveName(anova.effectNames{np}) ' + SnPMsub.tif']),'Resolution',imageResolution)
            savefig([savedir2 verifSaveName(anova.effectNames{np}) '/FIG/' verifSaveName(anova.effectNames{np}) ' + SnPMsub'])
            close

            plotmeanSnPMsub(posthoc{np}.data.continuum,posthoc{np}.tTests.Tcontinuum,posthoc{np}.tTests.Tsignificant,posthoc{np}.data.names,posthoc{np}.differences.names,CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,[],[],ratioSnPM,SnPMPos,aovColor,xLine,yLine,xGrid,yGrid)
            exportgraphics(gcf,fullfile(savedir2,  verifSaveName(anova.effectNames{np}), [verifSaveName(anova.effectNames{np}) ' + SnPMsubNoAOV.tif']),'Resolution',imageResolution)
        ...
        if numel(posthoc)==7
            ...
            for effectFixed=1:3
                ...
                for p=1:nPlot
                    ...
                    plotmean(posthoc{np}.data.continuum(whichPlot{p}),CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,xLine,yLine,xGrid,yGrid)
                    legend(posthoc{np}.data.names(whichPlot{p}),'Location','eastoutside','box','off')
                    exportgraphics(gcf,fullfile(savedir2,  verifSaveName([anova.effectNames{mainEffect(1)} ' x ' anova.effectNames{mainEffect(2)}]), verifSaveName(anova.effectNames{mainEffect(whichFixed(2,p))}),[verifSaveName(posthoc{mainEffect(whichFixed(1,p))}.data.names{whichModal(p)}) '.tif']),'Resolution',imageResolution)
                    ...
                    exportgraphics(gcf,fullfile(savedir2,  verifSaveName([anova.effectNames{mainEffect(1)} ' x ' anova.effectNames{mainEffect(2)}]), verifSaveName(anova.effectNames{mainEffect(whichFixed(2,p))}),[verifSaveName(posthoc{mainEffect(whichFixed(1,p))}.data.names{whichModal(p)}) ' +SnPM.tif']),'Resolution',imageResolution)
                    ...
                    exportgraphics(gcf,fullfile(savedir2,  verifSaveName([anova.effectNames{mainEffect(1)} ' x ' anova.effectNames{mainEffect(2)}]), verifSaveName(anova.effectNames{mainEffect(whichFixed(2,p))}),[verifSaveName(posthoc{mainEffect(whichFixed(1,p))}.data.names{whichModal(p)}) ' +SnPMnoAOV.tif']),'Resolution',imageResolution)
                    ...
                    exportgraphics(gcf,fullfile(savedir2,  verifSaveName([anova.effectNames{mainEffect(1)} ' x ' anova.effectNames{mainEffect(2)}]), verifSaveName(anova.effectNames{mainEffect(whichFixed(2,p))}),[verifSaveName(posthoc{mainEffect(whichFixed(1,p))}.data.names{whichModal(p)}) ' +SnPMsub.tif']),'Resolution',imageResolution)
                    ...
                    exportgraphics(gcf,fullfile(savedir2,  verifSaveName([anova.effectNames{mainEffect(1)} ' x ' anova.effectNames{mainEffect(2)}]), verifSaveName(anova.effectNames{mainEffect(whichFixed(2,p))}),[verifSaveName(posthoc{mainEffect(whichFixed(1,p))}.data.names{whichModal(p)}) ' +SnPMsubNoAOV.tif']),'Resolution',imageResolution)
        ...
        if numel(posthoc)==3
            for p=1:nPlot
                ...
                plotmean(posthoc{pos}.data.continuum(whichPlot{p}),CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,xLine,yLine,xGrid,yGrid)
                ...
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2,  verifSaveName(anova.effectNames{whichFixed(2,p)}), [verifSaveName(posthoc{whichFixed(1,p)}.data.names{whichModal(1,p)}) ' .tif']),'Resolution',imageResolution)
        ...
        elseif numel(posthoc)==7
            for p=1:nPlot
                ...
                legend(posthoc{pos}.data.names(whichPlot{p}(isEmptydata)),'Location','eastoutside','box','off')
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2, verifSaveName(anova.effectNames{whichFixed(1,p)}), [verifSaveName([posthoc{whichFixed(2,p)}.data.names{whichModal(1,p)}  ' x ' posthoc{whichFixed(3,p)}.data.names{whichModal(2,p)}]) ' .tif']),'Resolution',imageResolution)
        ...
        for p=1:nPlot
            ...
            if numel(posthoc)==3
                ...
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2, verifSaveName(anova.effectNames{whichFixed(2,p)}), [verifSaveName(posthoc{whichFixed(1,p)}.data.names{whichModal(1,p)}) ' + SnPM.tif']),'Resolution',imageResolution)
                ...
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2, verifSaveName(anova.effectNames{whichFixed(2,p)}), [verifSaveName(posthoc{whichFixed(1,p)}.data.names{whichModal(1,p)}) ' + SnPMnoAOV.tif']),'Resolution',imageResolution)
                ...
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2, verifSaveName(anova.effectNames{whichFixed(2,p)}), [verifSaveName(posthoc{whichFixed(1,p)}.data.names{whichModal(1,p)}) ' + SnPMsub.tif']),'Resolution',imageResolution)
                ...
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2, verifSaveName(anova.effectNames{whichFixed(2,p)}), [verifSaveName(posthoc{whichFixed(1,p)}.data.names{whichModal(1,p)}) ' + SnPMsubNoAOV.tif']),'Resolution',imageResolution)
                ...
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2, verifSaveName(anova.effectNames{whichFixed(1,p)}), [verifSaveName([posthoc{whichFixed(2,p)}.data.names{whichModal(1,p)} ' x ' posthoc{whichFixed(3,p)}.data.names{whichModal(2,p)}]) ' + SnPM.tif']),'Resolution',imageResolution)
                ...
                plotmeanSnPM(posthoc{pos}.data.continuum(whichPlot{p}),posthoc{pos}.tTests.Tcontinuum(:,whichCompare),posthoc{pos}.tTests.Tsignificant(:,whichCompare),posthoc{pos}.data.names  (whichPlot{p}(isEmptydata)),posthoc{pos}.differences.names(whichCompare),CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,[],[],ratioSnPM,SnPMPos,aovColor,xLine,yLine,xGrid,yGrid)
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2, verifSaveName(anova.effectNames{whichFixed(1,p)}), [verifSaveName([posthoc{whichFixed(2,p)}.data.names{whichModal(1,p)} ' x ' posthoc{whichFixed(3,p)}.data.names{whichModal(2,p)}]) ' + SnPMnoAOV.tif']),'Resolution',imageResolution)
                ...
                plotmeanSnPMsub(posthoc{pos}.data.continuum(whichPlot{p}),posthoc{pos}.tTests.Tcontinuum(:,whichCompare),posthoc{pos}.tTests.Tsignificant(:,whichCompare),posthoc{pos}.data.names  (whichPlot{p}(isEmptydata)),posthoc{pos}.differences.names(whichCompare),CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,...
                    anova.Fsignificant([whichFixed(1,p) nAnovaInt 7]),{anova.effectNames{whichFixed(1,p)},[anova.effectNames{nNames(1,1)} ' x ' anova.effectNames{nNames(1,2)}], [anova.effectNames{nNames(2,1)} ' x ' anova.effectNames{nNames(2,2)}],[anova.effectNames{1} ' x ' anova.effectNames{2} ' x ' anova.effectNames{3}]},ratioSnPM,SnPMPos,aovColor,xLine,yLine,xGrid,yGrid)
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2, verifSaveName(anova.effectNames{whichFixed(1,p)}), [verifSaveName([posthoc{whichFixed(2,p)}.data.names{whichModal(1,p)} ' x ' posthoc{whichFixed(3,p)}.data.names{whichModal(2,p)}]) ' + SnPMsub.tif']),'Resolution',imageResolution)
                ...
                plotmeanSnPMsub(posthoc{pos}.data.continuum(whichPlot{p}),posthoc{pos}.tTests.Tcontinuum(:,whichCompare),posthoc{pos}.tTests.Tsignificant(:,whichCompare),posthoc{pos}.data.names  (whichPlot{p}(isEmptydata)),posthoc{pos}.differences.names(whichCompare),CI,xlab,ylab,Fs,xlimits,nTicksX,nTicksY,clPlot,imageFontSize,imageSize,transparancy1D,ylimits,[],[],ratioSnPM,SnPMPos,aovColor,xLine,yLine,xGrid,yGrid)
                exportgraphics(gcf,fullfile(savedir, 'Post hoc', savedir2, verifSaveName(anova.effectNames{whichFixed(1,p)}), [verifSaveName([posthoc{whichFixed(2,p)}.data.names{whichModal(1,p)} ' x ' posthoc{whichFixed(3,p)}.data.names{whichModal(2,p)}]) ' + SnPMsubNoAOV.tif']),'Resolution',imageResolution)
...
if min(dimensions)>1
    ...
    for np=1:numel(posthoc)
        ...
        for combi=1:numel(posthoc{np}.data.meanContinuum)
            ...
            % means
            displayMeanMaps(posthoc{np}.data.meanContinuum{combi},Fs,xlab,ylab,ylimits,nTicksX,nTicksY,limitMeanMaps,xlimits,imageFontSize,imageSize,colorbarLabel,colorMap,equalAxis,deleteAxis)
            title(posthoc{np}.data.names{combi})
            exportgraphics(gcf,fullfile(savedir2,[verifSaveName(posthoc{np}.data.names{combi}) '.tif']),'Resolution',imageResolution)
            savefig([savedir2 '/FIG/' verifSaveName(posthoc{np}.data.names{combi})])
            close

            % sd
            displayMeanMaps(posthoc{np}.data.sdContinuum{combi},Fs,xlab,ylab,ylimits,nTicksX,nTicksY,[],xlimits,imageFontSize,imageSize,colorbarLabel,colorMap,equalAxis,deleteAxis)
            title(posthoc{np}.data.names{combi})
            exportgraphics(gcf,fullfile(savedir2, 'SD', [verifSaveName(posthoc{np}.data.names{combi}) '.tif']),'Resolution',imageResolution)
        ...
        for comp=1:numel(posthoc{np}.differences.names)
            ...
            % differences
            displayDiffMaps(posthoc{np}.differences.continuum{comp},Fs,xlab,ylab,ylimits,nTicksX,nTicksY,limitMeanMaps,xlimits,imageFontSize,imageSize,colorbarLabel,colorMapDiff,diffRatio,equalAxis,deleteAxis)
            if displayContour
            ...
            % relative differences
            displayRelativeDiffMaps(posthoc{np}.differences.continuumRelative{comp},Fs,xlab,ylab,ylimits,nTicksX,nTicksY,xlimits,imageFontSize,imageSize,colorMapDiff,relativeRatio,equalAxis,deleteAxis)
            if displayContour
                dispContour(abs(posthoc{np}.tTests.contourSignificant{comp}),posthoc{np}.tTests.Tthreshold{comp},contourColor,dashedColor,transparancy,lineWidth,lineStyle)
            end
            title(posthoc{np}.differences.names{comp})
            ...
            % SnPM analysis
            displayTtest(posthoc{np}.tTests.Tcontinuum{comp},posthoc{np}.tTests.Tthreshold{comp},[],Fs,xlab,ylab,ylimits,dimensions,nTicksX,nTicksY,xlimits,imageFontSize,imageSize,colorMapDiff,equalAxis,deleteAxis,statLimit)
            if displayContour
            ...
            % ES
            displayMapsES(posthoc{np}.differences.ES{comp},Fs,xlab,ylab,ylimits,nTicksX,nTicksY,xlimits,imageFontSize,imageSize,colorMap,equalAxis,deleteAxis)
            title(posthoc{np}.differences.names{comp})
            if displayContour
                dispContour(abs(posthoc{np}.tTests.contourSignificant{comp}),posthoc{np}.tTests.Tthreshold{comp},contourColor,dashedColor,transparancy,lineWidth,lineStyle)
            end
